<template>
  <div id="personalSetting">
    <div class="clear-cookie flex flex-clo">
      ...
      <button class="btn-def">{{langPersonal.clearBtn}}</button>
    </div>
    ...
    <div class="def-code flex flex-clo">
      ...
      <div v-for="(item, index) in defaultCode" :key="index" class="code flex flex-clo">
        ...
        <el-input class="code-input" type="textarea" :autosize="{ minRows: 2, maxRows: 4}" :placeholder="langPersonal.codePlaceholder"
          v-model="defaultCode[index]">
        </el-input>
      </div>
    </div>
  </div>
